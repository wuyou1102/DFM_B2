...
class Frame(A01_CIT_Base.Frame):
    def __init__(self):
        ...
        if self.__Sources is None:
            Utility.Alert.Warn(u"无法找到信号发生器，接收灵敏度测试无法自动测试")
        if self.__Analyzer is None:
            Utility.Alert.Warn(u"无法找到信号分析仪，发送功率测试无法自动测试")
    ...
    def __init_instrument(self):
        ...
        for resource in resources:
            ...
            if inst.model_name in ["N9020A"]:  # 信号分析仪
                ...
                if not self.__Analyzer.InitInstrumentSetting():
                    Utility.Alert.Error("信号分析仪初始化失败，请手动检查。")
            elif inst.model_name in ["N5172B"]:  # 信号发生器
                self.__Sources = SignalSources(instrument=inst)
                if not self.__Sources.InitInstrumentSetting():
                    Utility.Alert.Error("信号发生器初始化失败，请手动检查。")
...
class SignalSources(object):
    ...
    def InitInstrumentSetting(self):
        ...
        result = result and self.SetFileData(config.get("File", "WFM1:SR_SLOT_20M_16QAM_12_1P25X4_1T.DAT"))
    ...
    def Reset(self):
        command = '*RST'
        return self.__inst.execute_command(command)

    def SetFrequency(self, MHz=2400):
        command = "FREQ {value} MHz".format(value=MHz)
        return self.__inst.execute_command(command)
    ...
    def SetFileData(self, file_name='WFM1:SR_SLOT_20M_16QAM_12_1P25X4_1T.DAT'):
        command = ':SOURce:RADio:ARB:WAVeform "{file_name}"'.format(file_name=file_name)
        return self.__inst.execute_command(command)
